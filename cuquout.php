...
<button class="sg-btn sg-btn-primary"  onclick="history.back()" >上一頁</button> <button class="sg-btn sg-btn-primary" onclick="javascript:window.location.reload()" >重新整理</button> <!-- Onclick -->
	...
	<br><!-- 換行 --> 
    <br>	
	<button class="sg-btn sg-btn-flat-default"><a href="cuquout_add.php">新增資料</a></button>	
		

<table class="sg-table sg-table--responsive sg-table--bordered  sg-table--striped  sg-table--highlight">
<caption>商品報價表</caption>
<thead>
		<tr>
			<th>產品編號</th>
			<th>產品名稱</th>
...
<?php
   ...
   // mysqli_query($connection,"SET NAMES 'utf8'");//設定語系
	...
	while($row_result=mysqli_fetch_assoc($result)){
		...
		echo "<td>".$row_result["PD_Name"]."</td>";
		echo "<td>".$row_result["PD_Pr"]."</td>";
		echo "<td>".$row_result["PD_Note"]."</td>";

		//echo "<div class='sg-btn-group'>";
		//echo "<td><button class='sg-btn sg-btn-default'>";
		echo "<td><a href='cuquout_update.php?id=".$row_result["PD_No"]."'>修改</a> ";
		//echo "</button>";
		
		//echo "<button class='sg-btn sg-btn-default'>";
		echo "<a href='cuquout_delete.php?id=".$row_result["PD_No"]."'>刪除</a>";
		//echo "</button></td>";
		//echo "</div>";
		echo "</tr>";
	}
...
<table border="0" align="center">
  <tr>
    ...
    <td><a href="cuquout.php?page=1">第一頁</a></td>
    <td><a href="cuquout.php?page=<?php echo $num_pages-1;?>">上一頁</a></td>
    <?php } ?>
    <?php if ($num_pages < $total_pages) { // 若不是最後一頁則顯示 ?>
    <td><a href="cuquout.php?page=<?php echo $num_pages+1;?>">下一頁</a></td>
    <td><a href="cuquout.php?page=<?php echo $total_pages;?>">最後頁</a></td>
    <?php } ?>
  </tr>
</table>
<table border="0" align="center">
	  ...
	  <div class="demo-btn-paragraph">
  	  ...
  	  for($i=1;$i<=$total_pages;$i++){
			  ...
			  echo "<button class='sg-btn sg-btn-default sg-btn-circle'>";
  	  	      ...
  	  	      echo "<a href=\"cuquout.php?page={$i}\" style=\"text-decoration:none;color:red;\">{$i}</a> ";
