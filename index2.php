...
//UPDATE 報價表 INNER JOIN (客戶銷貨主表 INNER JOIN 應收帳款表 ON 客戶銷貨主表.[客戶編號] = 應收帳款表.[客戶編號]) 
//ON (報價表.[客戶編號] = 客戶銷貨主表.[客戶編號]) AND (報價表.[產品編號] = 客戶銷貨主表.[產品編號])
 
//SET 應收帳款表.本期出貨 = [應收帳款表]![本期出貨]+[客戶銷貨主表]![數量]*[報價表]![單價], 
//應收帳款表.累計結欠 = [應收帳款表]![累計結欠]+[客戶銷貨主表]![數量]*[報價表]![單價], 
//客戶銷貨主表.轉應收 = "T"
//WHERE (((客戶銷貨主表.轉應收) Is Null));

// TEST ~

//UPDATE cuquout INNER JOIN (splist INNER JOIN rcpay ON splist.[CU_No] = rcpay.[CU_No]) 
//ON (cuquout.[PD_Name] = splist.[PD_Name]) 
//SET rcpay.CR_Spamt = [rcpay]![CR_Spamt]+[Splist]![SP_Qty]*[cuquout]![PD_Pr], 
//rcpay.CR_Upamt = [rcpay]![CR_Upamt+[Splist]![SP_Qty]*[cuquout]![PD_Pr], 
//Splist.TR_Note = "T"
//WHERE (((Splist.TR_Note) Is Null));
...
// $sql_query = "UPDATE rcpay INNER JOIN cuquout,splist
// ON (cuquout.PD_No = splist.PD_No) 
// SET Splist.TR_Note = 'T' WHERE (((Splist.TR_Note) Is Null))";//TURE


//$sql_query = "UPDATE splist SET Splist.TR_Note = 'T' WHERE (((Splist.TR_Note) Is Null))";//TURE
//$sql_query = "UPDATE splist SET Splist.TR_Note = Null WHERE (((Splist.TR_Note) = 'T' ))";//TURE
