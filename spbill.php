...
<button class="sg-btn sg-btn-primary"  onclick="history.back()" >上一頁</button> <button class="sg-btn sg-btn-primary" onclick="javascript:window.location.reload()" >重新整理</button> <!-- Onclick -->
	...
	<br><!-- 換行 --> 
    <br>	
	<button class="sg-btn sg-btn-flat-default"><a href="spbill_add.php">新增資料</a></button>	
		

<table class="sg-table sg-table--responsive sg-table--bordered  sg-table--striped  sg-table--highlight">
<caption>銷貨單號表</caption>
<thead>
		<tr>
			<th>出貨單號</th>
			<th>客戶編號</th>
...
<?php
   ...
   // mysqli_query($connection,"SET NAMES 'utf8'");//設定語系
	...
	while($row_result=mysqli_fetch_assoc($result)){
		...
		echo "<td>".$row_result["SP_Date"]."</td>";
		echo "<td>".$row_result["UP_Time"]."</td>";
	
		//echo "<div class='sg-btn-group'>";
		//echo "<td><button class='sg-btn sg-btn-default'>";
		echo "<td><a href='spbill_update.php?id=".$row_result["SP_Blno"]."'>修改</a> ";
		//echo "</button>";
		
		//echo "<button class='sg-btn sg-btn-default'>";
		echo "<a href='spbill_delete.php?id=".$row_result["SP_Blno"]."'>刪除</a>";
		//echo "</button></td>";
		//echo "</div>";
		echo "</tr>";
	}
...
<table border="0" align="center">
  <tr>
    ...
    <td><a href="spbill.php?page=1">第一頁</a></td>
    <td><a href="spbill.php?page=<?php echo $num_pages-1;?>">上一頁</a></td>
    <?php } ?>
    <?php if ($num_pages < $total_pages) { // 若不是最後一頁則顯示 ?>
    <td><a href="spbill.php?page=<?php echo $num_pages+1;?>">下一頁</a></td>
    <td><a href="spbill.php?page=<?php echo $total_pages;?>">最後頁</a></td>
    <?php } ?>
  </tr>
</table>
<table border="0" align="center">
	  ...
	  <div class="demo-btn-paragraph">
  	  ...
  	  for($i=1;$i<=$total_pages;$i++){
			  ...
			  echo "<button class='sg-btn sg-btn-default sg-btn-circle'>";
  	  	      ...
  	  	      echo "<a href=\"spbill.php?page={$i}\" style=\"text-decoration:none;color:red;\">{$i}</a> ";
